...
(food_bal_total <- fread("FoodBalanceSheets_E_All_Data_(Normalized).csv", header = TRUE) %>% 
  ...
  filter(Year>2010) %>% 
   ...
   filter(Item %in% c("Cereals - Excluding Beer", "Fruits - Excluding Wine", "Vegetables", "Starchy Roots" , "Sugar & Sweeteners", "Animal fats", "Meat", "Fish, Seafood", "Milk - Excluding Butter", "Eggs")) %>% 
  group_by(Area, Year) %>% 
...
(gdp <- fread("GDP_per_cap.csv", header=TRUE) %>% 
...
ggplot(food_bal_animal, aes(x=(GDP), y=Value))+
  geom_point()+
  theme_pubr()+
  scale_x_log10()


sort(unique(food_bal_total$Item))
```
