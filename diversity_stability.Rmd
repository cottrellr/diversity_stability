...
consumption_gdp <- 
  ...
  labs(y=bquote(Meat~consumption~(kg~capita^-1~yr^-1)), x=bquote(GDP~capita^-1~(USD~2010~equivalents)))+
  ...
  geom_line(aes(y=predict(glm(Value~log10(GDP_per_cap), family="quasipoisson"), type="response"))) + 
   geom_text_repel(data=labels, mapping = aes(x=GDP_per_cap, y=Value, label=Area), force=0.9, segment.color = "grey50", point.padding = 0.1,, size=1.5)+
