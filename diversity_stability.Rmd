...
(food_bal_total <- fread("FoodBalanceSheets_E_All_Data_(Normalized).csv", header = TRUE) %>% 
   ...
   filter(!Area %in% c("China, Hong Kong SAR", "China, Macao SAR", "China, Taiwan Province of")) %>% 
  filter(Year>2010) %>% 
...
imports <- fread("imports_reexports_aus_seafood.csv", header=TRUE) %>% 
  filter(!Partner %in% c("World", "Australia")) %>% 
  rename(iso_a3 = `Partner ISO`, 
         Value = `Alt Qty Unit`, 
         Value2 = `Netweight (kg)`) %>% 
  select(Year, Partner, iso_a3, Value, Value2) %>% 
  mutate(Value=Value/1000, Value2=Value2/1000) %>% 
  group_by(Partner, iso_a3) %>% 
  summarise(Value=mean(Value))
