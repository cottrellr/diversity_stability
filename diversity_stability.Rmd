...
(seafood_demand_supply <- 
    bind_rows(
      aqua_fish_combo <- 
        ...
        select(Year, Value) %>% 
        mutate(Element = "Domestic production"),
      ...
      fish_consumption <- fread("FoodBalanceSheets_E_All_Data_(Normalized).csv", header = TRUE) %>% 
        ...
        select(Year, Value) %>% 
        mutate(Element="Domestic consumption")
    ) %>% 
    mutate(Element = factor(Element, levels=c("Domestic consumption",  "Domestic production"))) %>% 
    ...
    scale_color_manual(values = c("black", "#3288BD"))+
...
(aquaculture_sentiment <- 
  ...
  coord_flip()+
    ...
    labs(fill="Proportion of Sentiment")+
...
nimby <- fread("state_production.csv") %>% 
  ...
  geom_text_repel(aes(x=Prop_prod, y=Prop_neg, label=State), point.padding = 0.2, force = 1, max.iter = 1000,size=3)+
...
ggarrange(aus_aqua, aquaculture_sentiment, nimby,
          nrow = 1, ncol=3,
          labels = c("a", "b", "c"),
          widths = c(1,1,0.75),
          font.label = list(size=9))+
