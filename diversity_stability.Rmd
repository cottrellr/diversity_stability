...
(crops_plot <- 
  ...
  summarise(Value=sum(Value)) %>% 
    ...
    scale_y_continuous(labels = c(20,40,60,80,100))+
...
(livestock <- 
   ...
   geom_line()+
   geom_line(aes(y=GAM, group=Element), colour="grey20", size=0.25)+
   scale_color_manual(values = c("#56C667FF", "#94D840FF"))+
   theme_pubr()+
   theme(
     text=element_text(size=6),
...
unique(meat$Item)
meat %>% filter(Year==2017) %>% arrange(-Value)

livestock %>% filter(Item=="Milk, whole fresh cow")
