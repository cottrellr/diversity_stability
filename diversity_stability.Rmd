...
(national_var_no_aqua <- 
    ...
    drop_na() %>% 
    group_by(iso_code) %>% 
    summarise(Stability= 1-sd(Prop_change))
)
...
model_no_aqua <- (glm(Stability~H, data=no_aqua_div_var, family = "quasibinomial"))
summary(model_no_aqua)


no_aqua_plot <- 
  ggplot(data = no_aqua_div_var, aes(x=H, y=Stability))+
  geom_point(colour="blue", size=.75)+
  geom_line(aes(y=predict(model_no_aqua, type="response")))+
  scale_x_continuous(limits=c(0,4))+
  #scale_y_continuous(limits = c(0,0.3))+
  theme_pubr()+ 
  theme(text=element_text(size=7))+
  labs(x="Shannon-Weiner Index (H)", y="Production stability", title = "Without aquaculture")
...
(national_var_with_aqua <- 
    ...
    group_by(iso_code) %>% 
    nest() %>% 
    mutate(Prop_change = purrr::map(data, ~(1-((.$Annual_total-lag(.$Annual_total))/lag(.$Annual_total))))) %>% 
    unnest() %>% 
    drop_na() %>% 
    group_by(iso_code) %>% 
    summarise(Stability= 1-sd(Prop_change))
...
model_aqua <- (glm(Stability~H, data=aqua_div_var, family = "quasibinomial"))
summary(model_aqua)



aqua_plot <- 
  ggplot(data = aqua_div_var, aes(x=H, y=Stability))+
  geom_point(colour="blue", size=.75)+
...
fisheries_aqua_plot <- 
  ...
  geom_point(colour="blue", size=.75)+
  geom_line(aes(y=predict(model_fisheries_aqua, type="response")))+
  scale_x_continuous(limits=c(0,4))+
...
(national_var_no_aqua <- 
    ...
    drop_na() %>% 
    group_by(iso_code) %>% 
    summarise(Stability_wo_aqua= 1-sd(Prop_change))
)
...
(national_var_with_aqua <- 
    ...
    drop_na() %>% 
    group_by(iso_code) %>% 
    summarise(Value= 1-sd(Prop_change))
)
...
(aqua_no_aqua <- 
  left_join(no_aqua_div_var, aqua_div_var, by="iso_code") %>% 
    mutate(H_change = H-H_wo_aqua, Stability_change = Stability-Stability_wo_aqua)) 
