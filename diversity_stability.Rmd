...
trends <- ggplot(food_bal_australia, aes(x=Year, y=Relative_value, colour=Item))+
  geom_line(size=0.6)+
  theme_pubr()+
  scale_color_manual(values = c( "dodgerblue","grey20")) +#"#440154FF" "#56C667FF"
  theme(
    text = element_text(size=5),
...
(seafood_demand_supply <- 
    ...
    scale_color_manual(values = c("firebrick" , "dodgerblue4","dodgerblue"))+
...
(all_seafood_trade <- 
   ...
   mutate(iso_a3 = case_when(Partner == "Br. Virgin Isds" ~ "VGB",
    ...
    select(Partner, Value, Import_dependence, iso_a3, SDG_14, CPI) %>% 
