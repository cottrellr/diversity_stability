...
(crops_plot <- 
  ...
  summarise(Value=sum(Value)) %>% 
    ...
    geom_line(aes(y=predict(gam(Value~s(Year)))), colour="grey20", size=0.25)+
    theme_pubr()+
    theme(
      text=element_text(size=6)
    )+
    labs(y=bquote(Production~(million~tonnes)))+
...
(livestock <- 
    ...
    unnest() %>% 
    ggplot(aes(x=Year, y=Value, colour=Element, linetype=Line_type))+
    geom_line(size=0.5)+
    #geom_line(aes(y=GAM, group=Feature), colour="grey20", size=0.25)+
    scale_color_manual(values = c("#5DC863FF", "#21908CFF"))+
...
(seafood <- 
   ...
   mutate(Element= factor(Element, levels=c("Fisheries", "Aquaculture"))) %>% 
