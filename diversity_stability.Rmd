...
labels <- food_bal_animal %>% 
  slice(c(1:10, 14)) %>% 
  #filter(Area %in% c("Australia", "United States of America")) %>% 
...
consumption_gdp <- 
  ...
  geom_point(size=1, colour="dodgerblue")+
  theme_pubr()+
  theme(
  ...
  labs(y=bquote(Animal-based~food~consumption~(kg~capita^-1~yr^-1)), x=bquote(GDP~capita^-1~(USD~2010~equivalents)))+
  ...
  geom_line(aes(y=predict(glm(Value~log10(GDP_per_cap), family="quasipoisson"), type="response"))) + 
   geom_text_repel(data=labels, mapping = aes(x=GDP_per_cap, y=Value, label=Area), force=0.9, segment.color = "grey80", point.padding = NA, size=1.5)+
...
consumption_items <- ggplot(food_bal_australia %>% 
  ...
  ggsave("Figure 1a - relative consumption.tiff", dpi=600, device = "tiff", width=12, height=9, units="cm")+
  ggsave("Figure 1a - relative consumption.pdf", dpi=600, device = "pdf", width=12, height=9, units="cm")
...
(seafood_demand_supply <- 
    ...
    ggsave("Production_seafood.pdf", dpi=600, device = "pdf", width=12, height = 9, units = "cm")
)
...
#Julia's global supply plot
```{r}

(food_bal_world <- 
   bind_rows(
     vroom("FoodBalanceSheets_E_All_Data_(Normalized).csv") %>% 
       filter(Element %in% c("Food supply quantity (kg/capita/yr)")) %>% 
       filter(Area == c("World")) %>% 
       filter(`Item Code`!= 2948) %>% 
       filter(Item %in% c("Meat", "Fish, Seafood", "Cereals - Excluding Beer", "Milk - Excluding Butter", "Eggs", "Fruits - Excluding Wine", "Pulses", "Vegetables", "Starchy Roots")) %>% 
...
(consumption_item_global <- ggplot(food_bal_world %>% 
         mutate(Item = factor(Item, levels=c("Fish, Seafood", "Fruit, Vegetables",  "Meat", "Milk", "Cereals", "Eggs", "Total food"))), aes(x=Year, y=Relative_value, colour=Item))+
  geom_line(size=0.9)+
  scale_color_manual(values = rev(c("#D53E4F", "#FC8D59" ,"#FEE08B", "goldenrod", "#E6F598", "#99D594", "#3288BD")))+
  theme_pubr()+
  scale_x_continuous(limits=c(1960,2020))+
  scale_y_continuous(limits = c(0.95,2.25), breaks=seq(1, 2.25, by=0.25), labels = seq(1, 2.25, by=0.25))+
  theme(
    legend.position = c(0.25,0.8),
    legend.title = element_blank(),
    legend.text = element_text(size=11),
  ...
  labs(y=bquote(Relative~consumption~capita^-1))+
  ggsave("Figure  - relative consumption global.tiff", dpi=600, device = "tiff", width=12, height=9, units="cm")+
  ggsave("Figure  - relative consumption global.pdf", dpi=600, device = "pdf", width=12, height=9, units="cm")
)


foodbal <- vroom("FoodBalanceSheets_E_All_Data_(Normalized).csv")
...
for(i in window){
  ...
  assign(paste0("window", i), 
    ...
    #mutate(Residuals = (resid(gam(Value~s(Year))))) %>% diag(1:4)
    mutate(Differenced = c(0, diff(Value))) %>% 
...
(landings <- ggplot(fisheries %>% filter(Width ==0.5),
 ...
 #geom_line(aes(y=predict(gam(Value~s(Year)))), size=0.6, colour="grey60")+
    ...
    annotate("text", x=1951, y=2.7e+5,label="Fisheries landings", hjust=0, size=2.9)+
...
(residuals <- ggplot(fisheries %>% filter(Width ==0.5),
  ...
  #geom_line(aes(y=predict(gam(Value~s(Year)))), size=0.6, colour="grey60")+
    ...
    labs(y=bquote(Tonnes~(10^3)))+
    #annotation_custom(fishing_img, xmin = 1950, 1960, ymin = 1.9e+5, ymax=2.9e+5)+
   annotate("text", x=1951, y=39e+3,label="First-difference detrended", hjust=0, size=2.9)+
...
# (Variance <- ggplot(fisheries %>% filter(Width==0.5),
#        aes(x=Year)) +
...
# length(which(boot_mod$t>AR1_tau))/1000
# 
# 
# 
# 
# (AR1 <- ggplot(fisheries %>% filter(Width==0.5),
#        aes(x=Year)) +
...
#reliability investigation into trend test
  ...
  comp_fisheries <- fisheries %>% 
   ...
   ggarrange(landings_residuals, vars,ar1, comp_index, 
    ...
    ggsave("Figure - EWS_window_sensitivity.tiff", device = "tiff", dpi=600, width = 18, height = 18, units = "cm")+
    ggsave("Figure - EWS_window_sensitivity.pdf", device = "pdf", dpi=600, width = 18, height = 18, units = "cm")
...
tiff("Acf_differenced.tiff", width = 14, height = 10, units = "cm", res = 300)
pacf((fisheries %>% filter(Width==0.5))$Differenced, main="")
dev.off()
...
nimby <- fread("state_production.csv") %>% 
  ...
  geom_text_repel(aes(x=Prop_prod, y=Prop_neg, label=State), point.padding = 0.3, force = 1, max.iter = 1000,size=3)+
...
ggarrange(crops_plot,
          ...
          heights = c(1,0.8),
          font.label = list(size=7))+
  ggsave("Figure - SI crops.tiff", device="tiff", dpi=600, width=12, height=12, units = "cm")
