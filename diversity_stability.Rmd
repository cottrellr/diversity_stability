...
(landings <- ggplot(fisheries,
  ...
  geom_line(aes(y=predict(gam(Value~s(Year)))), size=0.5, colour="grey50")+
    ...
    labs(y="Fisheries landings (1000s tonnes)")+
...
ggarrange(landings, Variance, AR1, 
         nrow=3, ncol=1,
         heights = c(1.15,1,1.1),
         labels = c("A", "B", "C"),
         font.label = list(size=7),
         label.y = c(1, 1.05, 1.05),
         align = "v")+
  ggsave("Figure - EWS.tiff", device = "tiff", dpi=600, width = 5.5, height = 11, units = "cm")+
  ggsave("Figure - EWS.pdf", device = "pdf", dpi=600, width = 5.5, height = 11, units = "cm")
...
ggplot(data=all_seafood_trade %>% slice(1:21) %>% drop_na() , aes(x=reorder(Partner,-Import_dependence), y=Import_dependence))+
  ...
  labs(y="Seafood import dependence (%)", x="Trade partner", fill="Progress towards SDG 14\nrelative to Australia")+
