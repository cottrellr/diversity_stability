...
hist(food_bal_animal$Value)

(unique(food_bal_total$Item))
```
...
(food_bal_australia <- fread("FoodBalanceSheets_E_All_Data_(Normalized).csv", header = TRUE) %>% 
   filter(Element %in% c("Food supply quantity (kg/capita/yr)")) %>% 
...
trends <- ggplot(food_bal_australia, aes(x=Year, y=Relative_value, colour=Item))+
  geom_line(size=0.6)+
  theme_pubr()+
  scale_color_manual(values = c( "#440154FF", "#56C667FF")) +
  theme(
    text = element_text(size=5),
  ...
  labs(y=bquote(Relative~~Australian~consumption~capita^-1))+
...
```{r}
ggarrange(consumption_gdp, trends,
          labels=c("A", "B"),
          nrow = 2,
          ncol=1,
          ...
          heights = c(1,0.9),
          label.y = c(1,1.07))+
  ggsave("Figure - consumption trends.tiff", device="tiff", dpi=600, width = 6, height = 9, units = "cm")+
  ggsave("Figure - consumption trends.pdf", device="pdf", dpi=600, width = 6, height = 9, units = "cm")
...
(crops_plot <- 
  ...
  mutate(Element="Total crop\nproduction") %>% 
    ...
    scale_colour_manual(values ="#DCE318FF")+
...
(livestock <- 
    ...
    scale_color_manual(values = rev(c("#2A788EFF" ,"#22A884FF", "#7AD151FF")))+
...
(seafood <- 
   ...
   mutate(Element= factor(Element, levels=c("Fisheries landings", "Aquaculture\nproduction"))) %>%
   group_by(Element) %>% 
   nest() %>% 
   mutate(GAM = purrr::map(data, ~(predict(gam(.$Value~s(.$Year)))))) %>% 
   unnest() %>% 
   mutate(GAM = case_when(Element=="Aquaculture\nproduction" ~ NA_real_,
                          TRUE ~ GAM)) %>% 
   ggplot(aes(x=Year, y=Value, colour=Element))+
   geom_line()+
   ...
   scale_color_manual(values=c("#440154FF", "#3B528BFF")) + 
   labs(y=bquote(Biomass~(thousand~tonnes)))+
   ...
   ggsave("Production_fish.tiff", dpi=600, device = "tiff", width=5.5, height = 4, units = "cm")
)
...
(seafood_demand_supply <- 
    bind_rows(
      aqua_fish_combo <- 
        ...
        select(Year, Value) %>% 
        mutate(Element = "Capture & farmed production"),
      
      fisheries %>% 
        mutate(Element="Capture production only"),
      ...
      fish_consumption <- fread("FoodBalanceSheets_E_All_Data_(Normalized).csv", header = TRUE) %>% 
        ...
        mutate(Value = (Supply*Population)/1000) %>% 
        select(Year, Value) %>% 
        mutate(Element="Fish/Seafood consumption")
    ) %>% 
    mutate(Element = factor(Element, levels=c("Fish/Seafood consumption", "Capture & farmed production", "Capture production only"))) %>% 
    ...
    scale_color_manual(values = c("black" , "#3B528BFF","#440154FF"))+
    scale_linetype_manual(values=c(1,1,1))+
    scale_y_continuous(labels = c("0", "200","400","600"), breaks = c(0, 2e+5,4e+5,6e+5), limits = c(0,6.1e+5))+
    scale_x_continuous(breaks=c(1960,1980,2000,2020), labels = c("1960","1980","2000","2020"), limits = c(1960,2020))+
    annotate("segment",x=2013, xend=2013, y=260e+3, yend=590e+3,  arrow=arrow(length = unit(1, "mm"), ends = "both", type = "open"), size=0.3, colour="grey50")+
    annotate("text", x=2019, y=410e+3, label="354 217 \nMT",size=1.5)+
    ggsave("Production_seafood.tiff", dpi=600, device = "tiff", width=5.5, height = 4, units = "cm")
)
...
(landings <- ggplot(fisheries,
       aes(x=Year)) +
  geom_line(aes(y=Value), colour="#440154FF")+
  geom_line(aes(y=predict(gam(Value~s(Year)))), size=0.5, colour="grey50")+
    theme_pubr()+
    theme(
      text = element_text(size=6),
...
(Variance <- ggplot(fisheries,
       aes(x=Year)) +
  geom_line(aes(y=Variance), colour="#440154FF")+
    theme_pubr()+
    theme(
      text = element_text(size=6),
      axis.title.x = element_blank()
    ...
    scale_x_continuous(breaks=c(1960,1980,2000,2020), labels = c("1960","1980","2000","2020"), limits = c(1950,2020))+
    labs(y="Variance")+
    annotate("segment", x=1950, xend=1982, y=8e+3, yend=8e+3, lineend = "butt", arrow=arrow(length = unit(1.5, "mm"), ends = "both", type = "open"))+
    annotate("text", x=1966, y=8.7e+3, label="Rolling window", size=1.8)+
...
(AR1 <- ggplot(fisheries,
       aes(x=Year)) +
  geom_line(aes(y=AR1), colour="#440154FF")+
    theme_pubr()+
    theme(
      text = element_text(size=6)
    )+
    ...
    scale_x_continuous(breaks=c(1960,1980,2000,2020), labels = c("1960","1980","2000","2020"), limits = c(1950,2020))+
    labs(y="AR1", x="Year")+
    annotate("segment", x=1950, xend=1982, y=0.05, yend=0.05, lineend = "butt", arrow=arrow(length = unit(1.5, "mm"), ends = "both", type = "open"))+
    annotate("text", x=1966, y=0.1, label="Rolling window", size=1.8)+
...
ggarrange(landings, Variance, AR1, 
         ...
         label.y = c(1, 1, 1),
         align = "v")+
  ggsave("Figure - EWS.tiff", device = "tiff", dpi=600, width = 5.5, height = 9, units = "cm")+
  ggsave("Figure - EWS.pdf", device = "pdf", dpi=600, width = 5.5, height = 9, units = "cm")
...
(all_seafood_trade <- 
   ...
   mutate(iso_a3 = case_when(Partner == "Br. Virgin Isds" ~ "VGB",
    ...
    select(Partner, Value, Import_dependence, iso_a3, SDG_14) %>% 
...
ggplot(data=all_seafood_trade %>% slice(1:21) %>% drop_na() , aes(x=reorder(Partner,-Import_dependence), y=Import_dependence))+
...
sum((all_seafood_trade %>% slice(1:21) %>% drop_na())$Import_dependence)
...
#Headlines analysis 
```{r}
(ranks_ag <- fread("Headlines_ag.csv", header=TRUE) %>% 
  filter(!is.na(Sentiment_1)) %>%
  ...
  group_by(State) %>% 
   summarise(Sum = sum(Sentiment_1))
)


(AG_sentiment <- 
  fread("Headlines_ag.csv", header=TRUE) %>% 
  filter(!is.na(Sentiment_1)) %>% 
  mutate(Sentiment_total = case_when(Sentiment_1 == -1 ~ "Negative",
                                     Sentiment_1 == 0 ~ "Neutral",
                                     Sentiment_1 == 1 ~ "Positive")) %>% 
  group_by(State) %>% 
...
(ranks_fish <- 
    fread("Headlines - fisheries.csv", header=TRUE) %>% 
  filter(!is.na(Sentiment_1)) %>% 
  mutate(Sentiment_total = case_when(Sentiment_1 == -1 ~ "Negative",
                                     Sentiment_1 == 0 ~ "Neutral",
                                     Sentiment_1 == 1 ~ "Positive")) %>% 
...
(ranks_aqua <- 
  fread("Headlines - aquaculture.csv", header=TRUE) %>% 
  filter(!is.na(Sentiment_1)) %>% 
  mutate(Sentiment_total = case_when(Sentiment_1 == -1 ~ "Negative",
                                     Sentiment_1 == 0 ~ "Neutral",
                                     Sentiment_1 == 1 ~ "Positive")) %>% 
...
all_sentiment <- ggarrange(annotate_figure(
    ggarrange(AG_sentiment, aquaculture_sentiment, fisheries_sentiment,
              ...
              nrow = 1,
              ncol = 3),
    bottom =  text_grob("Proportion of news headlines", size = 10)),
  sentiment_legend,
  nrow = 2,
  ncol=1,
  heights = c(1,0.2))+
...
(AG_impacts <- ggplot(data= bind_rows(Impacts %>% 
  ...
  theme_pubr()+
    theme(
      ...
      axis.title.x = element_blank(),
      axis.text.x = element_text(angle=45, hjust=1),
      plot.margin = unit(c(t=0.4, r=0.2, b=0.55, l=0.3), "cm")
    )+
    labs(y="Normalised impact")+
    guides(fill=FALSE)+
